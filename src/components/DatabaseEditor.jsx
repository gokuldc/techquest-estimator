...
export default function DatabaseEditor({ onBack }) {
    ...
    const handleDeleteRegion = async (regionId, regionName) => {
        if (window.confirm(`Are you sure you want to delete the region "${regionName}"? All local rates for this region will be permanently erased.`)) {
            // 1. Delete the region from the database
            await db.regions.delete(regionId);
            
            // 2. Deep clean the resources table to remove the orphaned rates
            const allResources = await db.resources.toArray();
            const updates = [];
            for (const res of allResources) {
                if (res.rates && res.rates[regionName] !== undefined) {
                    const newRates = { ...res.rates };
                    delete newRates[regionName]; // Erase the specific region's rate
    ...
    return (
        <Box sx={{ maxWidth: 1200, mx: "auto", p: 3 }}>
            ...
            <Typography variant="h4" fontWeight="bold" mb={3}>Database Manager</Typography>
                    ...
                    <Grid container spacing={4}>
                        <Grid item xs={12} md={6}>
                            <Paper sx={{ p: 4, textAlign: 'center', height: '100%' }}>
                                ...
                                <Button variant="contained" size="large" onClick={exportDatabase} sx={{ mt: 2 }}>Download Master File</Button>
                            </Paper>
                        </Grid>
                        <Grid item xs={12} md={6}>
                            <Paper sx={{ p: 4, textAlign: 'center', height: '100%', border: '2px dashed', borderColor: 'error.main' }}>
                                ...
                                <Button variant="outlined" color="error" size="large" onClick={() => fileInputRef.current.click()} sx={{ mt: 2 }}>Upload Master File</Button>
                            </Paper>
                        </Grid>
                    </Grid>
                </Box>
                ...
                <Box>
                        ...
                        <Grid item xs={12} md={6}>
                            <Paper sx={{ p: 3, height: '100%' }}>
                                ...
                                <Box display="flex" gap={2}>
                                    ...
                                    <Button variant="contained" onClick={handleAddRegion} disableElevation>Add</Button>
                                </Box>
                                
                                {/* Dynamic Chip list for deleting regions */}
                                {regions.length > 0 && (
                        ...
                        <Grid container spacing={3}>
                            ...
                            <Grid item xs={12} md={6}>
                                ...
                                <Box display="flex" gap={1}>
                                    ...
                                    <Button variant="contained" onClick={addResourceManually} disableElevation>Add</Button>
                                </Box>
                            </Grid>
                        </Grid>
                    </Paper>
                    ...
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                        ...
                        <Box display="flex" gap={1} alignItems="center">
                            <Button size="small" variant="outlined" disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)}>Prev</Button>
                            <Typography variant="body2" fontWeight="bold">Page {currentPage} of {totalPages || 1}</Typography>
                            <Button size="small" variant="outlined" disabled={currentPage === totalPages || totalPages === 0} onClick={() => setCurrentPage(p => p + 1)}>Next</Button>
                        </Box>
                    </Box>
                        ...
                        <Table stickyHeader size="small">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell onClick={() => handleSort('code')} sx={{ cursor: 'pointer', width: 120 }}>Code {sortField === 'code' ? (sortAsc ? '↑' : '↓') : ''}</TableCell>
                                    <TableCell onClick={() => handleSort('description')} sx={{ cursor: 'pointer' }}>Description {sortField === 'description' ? (sortAsc ? '↑' : '↓') : ''}</TableCell>
                                    <TableCell sx={{ width: 100 }}>Unit</TableCell>
                                    {regions.map(r => <TableCell key={r.id} sx={{ color: 'primary.main', fontWeight: 'bold', width: 120 }}>{r.name}</TableCell>)}
                                </TableRow>
                            </TableHead>
                            <TableBody>
            ...
            {tab === "createBoq" && (
                <Paper sx={{ p: 4 }}>
                    <Typography variant="h6" fontWeight="bold" mb={3} borderBottom="1px solid" borderColor="divider" pb={1}>
                    ...
                    <Button variant="outlined" onClick={addSpreadsheetRow} sx={{ mb: 4 }}>+ Add Component Row</Button>

                    <Box display="flex" justifyContent="flex-end">
                        <Paper sx={{ width: 400, p: 3, bgcolor: 'action.hover' }} elevation={0} variant="outlined">
                            ...
                            <Button variant="contained" color="success" fullWidth size="large" onClick={saveMasterBoq} startIcon={<SaveIcon />} disableElevation>
            ...
            {tab === "viewBoq" && (
                <Box>
                    ...
                    <Grid container spacing={2}>
                        {masterBoqs.length === 0 && <Grid item xs={12}><Typography color="text.secondary">No items created yet.</Typography></Grid>}
                        {masterBoqs.map(b => (
                            <Grid item xs={12} key={b.id}>
                                <Paper sx={{ p: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Box>
                                        <Typography variant="h6"><Box component="span" fontWeight="bold">{b.itemCode ? `[${b.itemCode}] ` : ''}</Box>{b.description} <Box component="span" color="primary.main">({b.unit})</Box></Typography>
                                        <Typography variant="body2" color="text.secondary" mt={0.5}>
                                            Components: {b.components.length} | Overhead: {b.overhead}% | Profit: {b.profit}%
                                        </Typography>
                                    </Box>
                                    <Box display="flex" gap={2}>
                                        <Button variant="outlined" color="warning" startIcon={<EditIcon />} onClick={() => editMasterBoq(b)}>Edit</Button>
                                        <Button variant="outlined" color="error" startIcon={<DeleteIcon />} onClick={() => deleteMasterBoq(b.id)}>Delete</Button>
                                    </Box>
                                </Paper>
                            </Grid>
                        ))}
